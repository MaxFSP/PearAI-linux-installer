#!/usr/bin/env bash

get_sudo_user() {
  logname || whoami
}

check_unfree() {
  local unfree_count=$(sudo grep -c "^\s*nixpkgs\.config\.allowUnfree\s*=\s*true\s*;" /etc/nixos/configuration.nix)
  local unfree_inside_count=$(sudo grep -c "^\s*nixpkgs\.config\.allowUnfree\s*=\s*true\s*;" /etc/nixos/configuration.nix | awk '/^{/ {count++} {print count}')

  if [ $unfree_count -gt 0 ]; then
      if [ $unfree_inside_count -eq 1 ]; then
          echo "Unfree packages are enabled system-wide"
          return 0
      else
          echo "Unfree packages setting exists but is not properly configured"
          return 2
      fi
  else
      echo "Unfree packages are not enabled"
      return 1
  fi
}

enable_unfree_and_steam() {
  # Backup configuration if backup doesn't exist
  if [ ! -f /etc/nixos/configuration.nix.backup ]; then
      sudo cp /etc/nixos/configuration.nix /etc/nixos/configuration.nix.backup
  fi

  # Create a temporary file
  local temp_file=$(mktemp)

  # Remove all existing unfree settings and steam-run
  sudo grep -v -E "nixpkgs\.config\.allowUnfree\s*=\s*true|steam-run" /etc/nixos/configuration.nix > "$temp_file"

  # Find the line number of the packages declaration
  local pkgs_line=$(grep -n "environment\.systemPackages = with pkgs;" "$temp_file" | cut -d: -f1)

  if [ -n "$pkgs_line" ]; then
      # Insert the unfree setting before the packages line
      sed -i "${pkgs_line}i\  nixpkgs.config.allowUnfree = true;" "$temp_file"
      # Add steam-run to the packages list
      sed -i "/environment\.systemPackages = with pkgs;/a\    steam-run" "$temp_file"
  else
      # If no packages line exists, add both settings before the closing brace
      sed -i '/^}/i\  nixpkgs.config.allowUnfree = true;\n  environment.systemPackages = with pkgs; [\n    steam-run\n  ];' "$temp_file"
  fi

  # Copy the modified content back to configuration.nix
  sudo cp "$temp_file" /etc/nixos/configuration.nix

  # Clean up
  rm "$temp_file"

  # Verify the changes
  if sudo grep -q "nixpkgs\.config\.allowUnfree\s*=\s*true" /etc/nixos/configuration.nix && \
     sudo grep -q "steam-run" /etc/nixos/configuration.nix; then
      echo "Successfully enabled unfree packages and added steam-run"
      return 0
  else
      echo "Failed to make required changes"
      sudo cp /etc/nixos/configuration.nix.backup /etc/nixos/configuration.nix
      return 1
  fi
}

check_steam_run() {
  if nix-store -qR /run/current-system | grep -q steam-run; then
      echo "steam-run is installed system-wide"
      return 0
  fi

  if nix-env -q | grep -q steam-run; then
      echo "steam-run is installed in user profile"
      return 0
  fi

  echo "steam-run is not installed"
  return 1
}

install_steam_run_system() {
  # Check if steam-run is already in configuration
  if sudo grep -q "steam-run" /etc/nixos/configuration.nix; then
      echo "steam-run is already in configuration.nix"
      return 0
  fi

  # Backup configuration
  if [ ! -f /etc/nixos/configuration.nix.backup ]; then
      sudo cp /etc/nixos/configuration.nix /etc/nixos/configuration.nix.backup
  fi

  # Create a temporary file
  local temp_file=$(mktemp)
  sudo cp /etc/nixos/configuration.nix "$temp_file"

  # Check for existing systemPackages section
  if grep -q "environment\.systemPackages = with pkgs;" "$temp_file"; then
      # Add steam-run to existing package list if not already present
      sed -i '/environment\.systemPackages = with pkgs;/a\    steam-run' "$temp_file"
  else
      # Add new package list with steam-run
      sed -i '/^}/i\  environment.systemPackages = with pkgs; [\n    steam-run\n  ];' "$temp_file"
  fi

  # Copy the modified content back
  sudo cp "$temp_file" /etc/nixos/configuration.nix

  # Clean up
  rm "$temp_file"

  if [ $? -eq 0 ]; then
      echo "Successfully added steam-run to system packages"
      return 0
  else
      echo "Failed to add steam-run to system packages"
      sudo cp /etc/nixos/configuration.nix.backup /etc/nixos/configuration.nix
      return 1
  fi
}

install_steam_run_user() {
  local username="$(get_sudo_user)"
  echo "Adding steam-run to user ${username} packages..."

  if [ -z "${username}" ] || ! id "${username}" >/dev/null 2>&1; then
      echo "Error: Invalid username"
      return 1
  fi

  # Create or update ~/.config/nixpkgs/config.nix for the user
  local user_nixpkgs_dir="/home/${username}/.config/nixpkgs"
  local user_config_file="${user_nixpkgs_dir}/config.nix"

  # Create directory if it doesn't exist
  sudo -u "${username}" mkdir -p "${user_nixpkgs_dir}"

  # Create or update config.nix
  if [ ! -f "${user_config_file}" ]; then
      sudo -u "${username}" bash -c "cat > '${user_config_file}'" << 'EOF'
{
allowUnfree = true;
}
EOF
  fi

  # Set proper permissions
  sudo chown -R "${username}:users" "${user_nixpkgs_dir}"

  echo "Attempting to install steam-run for user ${username}..."
  if sudo -u "${username}" nix-env -iA nixos.steam-run; then
      echo "Successfully installed steam-run for user ${username}"
      return 0
  else
      echo "Failed to install steam-run for user ${username}"
      return 1
  fi
}

check_configuration_file() {
  if [ ! -f /etc/nixos/configuration.nix ]; then
      echo "Error: configuration.nix not found in /etc/nixos/"
      exit 1
  fi
}

check_sudo_privileges() {
  if ! sudo -v; then
      echo "Error: This script requires sudo privileges"
      exit 1
  fi
}

handle_unfree_packages() {
  check_unfree
  local unfree_status=$?

  case $unfree_status in
      0)
          echo "Unfree packages are already enabled, proceeding..."
          install_steam_run_system
          ;;
      1|2)
          echo "Unfree packages must be enabled to install steam-run."
          read -p "Would you like to enable unfree packages and install steam-run? (y/n): " enable_choice

          if [[ $enable_choice =~ ^[Yy]$ ]]; then
              enable_unfree_and_steam
          else
              echo "Cannot proceed without enabling unfree packages."
              exit 1
          fi
          ;;
  esac
}

rebuild_system() {
  echo "Rebuilding NixOS configuration..."
  if sudo nixos-rebuild switch; then
      echo "System successfully rebuilt"
      return 0
  else
      echo "Error: Failed to rebuild system"
      return 1
  fi
}

main() {
  if [ ! -f /etc/NIXOS ]; then
      echo "Error: This script is intended for NixOS only"
      exit 1
  fi

  check_sudo_privileges
  check_configuration_file

  check_steam_run
  local steam_run_status=$?

  if [ $steam_run_status -eq 0 ]; then
      echo "steam-run is already installed."
      exit 0
  fi

  echo "Where would you like to install steam-run?"
  echo "1) System-wide (available to all users)"
  echo "2) User profile (only for current user)"
  echo "3) Cancel installation"
  read -p "Enter your choice (1/2/3): " choice

  case $choice in
      1)
          echo "Installing steam-run system-wide..."
          handle_unfree_packages

          rebuild_system && {
              echo "Installation completed successfully!"
              echo "You may need to log out and back in for the changes to take effect."
              exit 0
          }
          ;;
      2)
          echo "Installing steam-run in user profile..."
          install_steam_run_user && {
              echo "Installation completed successfully!"
              echo "You may need to log out and back in for the changes to take effect."
              exit 0
          }
          ;;
      3)
          echo "Installation cancelled."
          exit 0
          ;;
      *)
          echo "Invalid choice. Exiting."
          exit 1
          ;;
  esac

  echo "Installation failed."
  exit 1
}
main
